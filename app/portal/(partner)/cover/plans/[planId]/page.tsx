      ...
      <div className="mt-4 flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] text-accent-ink/55">
            ext id <span className="font-mono">{plan.externalId}</span> · billed{" "}
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/portal/cover/plans/${plan.id}/edit`}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-black/[0.12] text-[13px] text-accent-ink hover:border-black/30"
          >
            <Pencil size={13} /> Edit
          </Link>
                ...
                <div className="rounded-xl border border-black/[0.08] overflow-hidden">
                  <table className="w-full text-[12px]">
                    <thead className="bg-black/[0.03] text-accent-ink/55 uppercase tracking-[0.1em] text-[10px]">
                      <tr>
                        <th className="text-left px-3 py-2 font-medium">Age range</th>
                  ...
                  {BENEFIT_ORDER.map(({ key, label }) => {
                    ...
                    return (
                      <tr key={key}>
                        ...
                        <td className="px-3 py-2 text-right font-mono text-accent-ink">
                          {item.unlimited ? "—" : formatKobo(item.limit ?? null)}
                        </td>
                        <td className="px-3 py-2 text-right text-accent-ink">
                          {item.co_pay_percent != null ? `${item.co_pay_percent}%` : "—"}
                        </td>
                        <td className="px-3 py-2 text-right text-accent-ink/65">
                          {item.waiting_period_days ? `${item.waiting_period_days}d` : "—"}
                        </td>
                      </tr>
                    );
                  })}
              ...
              <div className="rounded-xl border border-black/[0.08] p-4">
                ...
                <dl className="mt-2 space-y-1.5 text-[13px]">
                  ...
                  {waitingPeriods.maternity != null ? (
                    <Row label="Maternity" value={`${waitingPeriods.maternity}d`} />
                  ) : null}
                  {waitingPeriods.pre_existing != null ? (
                    <Row label="Pre-existing" value={`${waitingPeriods.pre_existing}d`} />
                  ) : null}
                </dl>
              </div>
            ) : null}
        ...
        <aside className="space-y-4 text-[13px]">
          <div className="rounded-xl border border-black/[0.08] p-4">
            ...
            <dl className="mt-3 space-y-2.5">
              <Row label="Last synced" value={new Date(plan.lastSyncedAt).toLocaleString()} />
              <Row
                label="Last verified"
                value={plan.lastVerifiedAt ? new Date(plan.lastVerifiedAt).toLocaleString() : "never"}
              />
              <Row
                label="Stale after"
                value={plan.staleAfter ? new Date(plan.staleAfter).toLocaleString() : "—"}
              />
            </dl>
          </div>
                  ...
                  <li key={ev.id} className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-accent-ink/75">
                        ...
                        {ev.changed ? (
                          <span className="ml-1 text-[#7a4a00]">(changed)</span>
                        ) : null}
                      </p>
          ...
          <div className="rounded-xl border border-black/[0.08] p-4">
            ...
            <dl className="mt-3 space-y-2.5">
              ...
              <Row label="Updated" value={new Date(plan.updatedAt).toLocaleDateString()} />
              {plan.effectiveFrom ? (
                <Row label="Effective from" value={new Date(plan.effectiveFrom).toLocaleDateString()} />
              ) : null}
              {plan.effectiveTo ? (
                <Row label="Effective to" value={new Date(plan.effectiveTo).toLocaleDateString()} />
              ) : null}
            </dl>
          </div>
        </aside>
